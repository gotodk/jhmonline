...
public class NoReSet_160505000040
{
    ...
    public DataSet NRS_EDIT(DataTable parameter_forUI)
    {
        ...
        param.Add("@CSID", ht_forUI["idforedit"].ToString());
        param.Add("@CSjiange", ht_forUI["CSjiange"].ToString());
        param.Add("@CSshang1", ht_forUI["CSshang1"].ToString());
        param.Add("@CSshang2", ht_forUI["CSshang2"].ToString());
    ...
    public DataSet NRS_EDIT_INFO(DataTable parameter_forUI)
    {
        ...
        return_ht = I_DBL.RunParam_SQL("select top 1 * from ZZZ_kaoqin_cs where CSID=@CSID", "数据记录", param);
