...
public class NoReSet_170523000070
{
    ...
    public DataSet NRS_EDIT(DataTable parameter_forUI)
    {
        ...
        param.Add("@H_ff_wanchengzt", ht_forUI["H_ff_wanchengzt"].ToString());
        param.Add("@H_ff_zj", ht_forUI["H_ff_zj"].ToString());
        param.Add("@H_ff_wcsj", ht_forUI["H_ff_wcsj"].ToString());
 

        if (ht_forUI.Contains("allpath_file1"))
        {
            param.Add("@H_ff_zhaopian", ht_forUI["allpath_file1"].ToString());
        }
        else
        {
            param.Add("@H_ff_zhaopian", "");
        }

 

        alsql.Add("UPDATE ZZZ_huodong SET H_ff_wanchengzt=@H_ff_wanchengzt,H_ff_zj=@H_ff_zj,H_ff_wcsj=@H_ff_wcsj,H_ff_zhaopian=@H_ff_zhaopian  where HID=@HID   ");
    ...
    public DataSet NRS_EDIT_INFO(DataTable parameter_forUI)
    {
        ...
        if ((bool)(return_ht["return_float"]))
        {
            ...
            //如果图片不是空值，把图片也弄个表加进来
            if (redb.Rows[0]["H_ff_zhaopian"].ToString() != "")
            {
                //Ttupianpath
                DataTable dttu = new DataTable("图片记录");
                dttu.Columns.Add("Ttupianpath");
                string[] arr_tu = redb.Rows[0]["H_ff_zhaopian"].ToString().Split(',');
