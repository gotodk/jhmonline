...
public class NoReSet_160505000039
{
    ...
    public DataSet NRS_ADD(DataTable parameter_forUI)
    {
        ...
        //以可排序guid方式生成
        string guid = CombGuid.GetNewCombGuid("DD");
        param.Add("@DDID", guid);
        param.Add("@DDbiaojiming", ht_forUI["DDbiaojiming"].ToString());
        param.Add("@DDzuobiao", ht_forUI["DDzuobiao"].ToString());
        param.Add("@DDdizhi", ht_forUI["DDdizhi"].ToString());
        param.Add("@DDfanwei", ht_forUI["DDfanwei"].ToString());

        alsql.Add("INSERT INTO ZZZ_kaoqin_dd(DDID, DDbiaojiming, DDzuobiao,DDdizhi,DDfanwei ) VALUES(@DDID, @DDbiaojiming, @DDzuobiao,@DDdizhi,@DDfanwei)");
    ...
    public DataSet NRS_EDIT(DataTable parameter_forUI)
    {
        ...
        ArrayList alsql = new ArrayList();
        Hashtable param = new Hashtable();
        param.Add("@DDID", ht_forUI["idforedit"].ToString());
        param.Add("@DDbiaojiming", ht_forUI["DDbiaojiming"].ToString());
        param.Add("@DDzuobiao", ht_forUI["DDzuobiao"].ToString());
        param.Add("@DDdizhi", ht_forUI["DDdizhi"].ToString());
        param.Add("@DDfanwei", ht_forUI["DDfanwei"].ToString());

        alsql.Add("UPDATE ZZZ_kaoqin_dd SET  DDbiaojiming=@DDbiaojiming, DDzuobiao=@DDzuobiao,DDdizhi=@DDdizhi,DDfanwei=@DDfanwei  where DDID=@DDID ");
    ...
    public DataSet NRS_EDIT_INFO(DataTable parameter_forUI)
    {
        ...
        return_ht = I_DBL.RunParam_SQL("select top 1 * from ZZZ_kaoqin_dd where DDID=@DDID", "数据记录", param);
