...
public class NoReSet_170522000016
{
    ...
    public DataSet NRS_ADD(DataTable parameter_forUI)
    {
        ...
        //以可排序guid方式生成
        string guid = CombGuid.GetMewIdFormSequence("HH_zmgs");
        param.Add("@ZID", guid);
        param.Add("@Ztitle", ht_forUI["Ztitle"].ToString());
        param.Add("@Zbeizhu", ht_forUI["Zbeizhu"].ToString());
        param.Add("@Zaddtime", ht_forUI["Zaddtime"].ToString());
        param.Add("@Ztjr", ht_forUI["yhbsp_session_uer_UAid"].ToString());
        param.Add("@Zshow", ht_forUI["Zshow"].ToString());
        if (ht_forUI.Contains("allpath_file1"))
        {
            param.Add("@Ztupian", ht_forUI["allpath_file1"].ToString());
        }
        else
        {
            param.Add("@Ztupian", "");
        ...
        alsql.Add("INSERT INTO HH_zmgs(ZID, Ztitle, Zbeizhu, Ztupian, Zaddtime, Ztjr,Zshow) VALUES(@ZID, @Ztitle, @Zbeizhu, @Ztupian, @Zaddtime, @Ztjr,@Zshow)");
    ...
    public DataSet NRS_EDIT(DataTable parameter_forUI)
    {
        ...
        alsql.Add("UPDATE HH_zmgs SET  Ztitle=@Ztitle, Zbeizhu=@Zbeizhu, Ztupian=@Ztupian, Zaddtime=@Zaddtime,Zshow=@Zshow where ZID=@ZID   ");
    ...
    public DataSet NRS_EDIT_INFO(DataTable parameter_forUI)
    {
        ...
        return_ht = I_DBL.RunParam_SQL("select  top 1  ZID, Ztitle, Zbeizhu, Ztupian, Zaddtime, Ztjr,Zshow  from HH_zmgs where ZID=@ZID", "数据记录", param);
        ...
        if ((bool)(return_ht["return_float"]))
        {
            ...
            //如果图片不是空值，把图片也弄个表加进来
            if (redb.Rows[0]["Ztupian"].ToString() != "")
            {
                //Ttupianpath
                DataTable dttu = new DataTable("图片记录");
                dttu.Columns.Add("Ttupianpath");
                string[] arr_tu = redb.Rows[0]["Ztupian"].ToString().Split(',');
