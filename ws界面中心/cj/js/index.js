...
function huoqu2() {
    ...
    while(jx)
    {
        ...
        if ($(".classtt_" + a).length < 1) {
            jx = false;
             
        }
        if ($(".img-chosed").length == 7) {
            jx = false;
...
function start() {
    ...
    //$(".btn-primary").text($(".img-chosed").length + "--" + c);
    if ($(".img-chosed").length >= c) {
        return;
...
function stop(){
   ...
   $('#box2').css("background", "url('" + src_tu + "') 50% 50%");
    $('#is-picture').append("<img class='img-chosed classtt_" + a + "' src=\"" + src_tu + "\" id=\"img-sdf1\" width=\"13%\" height=\"100%\" />");
